...
export default function CustomPaginationActionsTable({
  ...
  const handleMLModel = async (obj,filename,format,type) => {
    ...
    const updatedResponse = await dbObj.updateResults(obj,data,user?._delegate?.uid);
    setReportData(updatedResponse);
  }
          ...
          {rows.map((row) => (
            <TableRow key={row.name}>
              ...
              <TableCell style={{ width: 160 }} align="right">
                <Button variant="outlined"><a href={row.url}>
                  See Image
                </a></Button>
              </TableCell>
              {(row.result==='No Diagnosis') ? ( 
              <TableCell style={{ width: 160 }} align="right">
                <div>{row.result}</div>
              </TableCell>
              )
            :
            <TableCell style={{ width: 160 }} align="right">
                <div className={`${(row.result==='Prediction: Normal') ? 'neg-res' : 'pos-res'}`}>{row.result}</div>
              </TableCell>
            }
            </TableRow>
          ))}
